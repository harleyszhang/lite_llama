"""GPU tests: the linear dispatch adapters forward to the right Triton kernel.

The kernels themselves are covered in ``test_quantization.py``; these tests
pin the *adapter layer* — the scheme entry points behind the LinearOp
signature must reproduce the direct kernel calls bit for bit (they are pure
argument rewrites), and ``run_quant_linear`` must select and run the same
...
from lite_llama.kernels.impls.native.linear_triton import (
    linear_w4a16,
    linear_w8a8_fp8,
    linear_w8a8_int8,
    linear_w8a16,
