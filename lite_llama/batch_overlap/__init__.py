...
* :mod:`~lite_llama.batch_overlap.overlap` — the host↔device axis (L1):
...
* :mod:`~lite_llama.batch_overlap.operations` — the stage/yield interleaving
  primitives (:class:`YieldOperation`, :class:`StateDict`,
  :func:`execute_overlapped_operations`).
* :mod:`~lite_llama.batch_overlap.comm_overlap` — the comm-stream plumbing
...
* :mod:`~lite_llama.batch_overlap.two_batch_overlap` — the L2 decode
  ping-pong executor and its policy.
* :mod:`~lite_llama.batch_overlap.operations_strategy` — per-layer operation
  streams: the layers' own bound methods, ordered with the yields placed
  (DeepSeek EP+TBO follows sglang's decode strategy).

Usage:
    from lite_llama.batch_overlap import deferred_all_reduce, StateDict
    from lite_llama.batch_overlap.two_batch_overlap import tbo_policy
