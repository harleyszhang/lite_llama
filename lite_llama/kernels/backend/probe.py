...
:meth:`~lite_llama.kernels.dispatcher.spec.KernelSpec.validate`), and
:func:`~lite_llama.kernels.dispatcher.dispatch` calls it before considering
the row. That is what makes a missing wheel a *ranking* event instead of a
crash: the row drops out, ``explain`` says why, and the native Triton floor
serves the call. This module holds both halves of that answer —
:func:`library_present`, the probe those rows point at, and
:class:`BackendInstall`, the recipe to report when a backend is absent but
wanted.
...
on first touch, so "the package directory exists" and "the package loads on
this driver" are different questions and dispatch only cares about the second.
The cost is bounded: at most one import per module name per process, paid on
the first dispatch that could have used the backend, and a box that never
dispatches that op never pays it at all.
