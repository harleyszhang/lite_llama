...
class Executor(ABC):
    ...
    def timeline_summary(self) -> str:
        ...
        Empty unless ``LITE_LLAMA_OVERLAP_TIMELINE`` is on, so callers print it
...
class UniProcExecutor(Executor):
    ...
    Args:
        engine: Built :class:`~lite_llama.engine.llm_engine.LLMEngine`; the executor
        ...
        pipeline: Worker feeds decode inputs back on the device (O2); ``None`` defers
            to :data:`~lite_llama.executor.worker.PIPELINE_ENV`.
    """
...
class MultiprocExecutor(Executor):
    ...
    Args:
        ...
        pipeline: Worker feeds decode inputs back on the device (O2); ``None`` defers
            to :data:`~lite_llama.executor.worker.PIPELINE_ENV` (how followers learn
            the driver's choice).
    """
...
def launch_tensor_parallel(
    ...
    :mod:`lite_llama.distributed.parallel_state`, not an argument).
