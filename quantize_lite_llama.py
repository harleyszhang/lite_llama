...
Author: AI Assistant (2025-01-22)
"""
...
class CalibrationDataLoader:
    ...
    def _load_calibration_data(self) -> List[str]:
        ...
        log.info(f"加载了 {len(texts)} 个校准样本")
...
def create_default_calibration_data(tokenizer_path: str, num_samples: int = 32) -> List[str]:
    ...
    log.info(f"使用默认校准数据，共 {len(texts)} 个样本")
...
def validate_quantization_config(method: str, config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method == QuantizationType.GPTQ:
        ...
        if validated_config['w_bit'] not in [2, 3, 4, 8]:
            raise ValueError(f"GPTQ不支持的位数: {validated_config['w_bit']}")
        ...
        if validated_config['w_bit'] not in [4, 8]:
            raise ValueError(f"AWQ不支持的位数: {validated_config['w_bit']}")
        ...
        if not (0.0 <= validated_config['alpha'] <= 1.0):
            raise ValueError(f"SmoothQuant的alpha参数必须在0-1之间: {validated_config['alpha']}")
...
def main():
    parser = argparse.ArgumentParser(description="量化lite_llama格式的模型")
    ...
    parser.add_argument("--group-size", type=int, default=128,
                        help="组大小 (default: 128)")

    # AWQ特有参数
    parser.add_argument("--alpha", type=float, default=0.5,
                        help="AWQ/SmoothQuant的alpha参数 (default: 0.5)")
    ...
    parser.add_argument("--w-bits", type=int, default=8,
                        help="权重量化位数 (SmoothQuant, default: 8)")
    parser.add_argument("--a-bits", type=int, default=8,
                        help="激活量化位数 (SmoothQuant, default: 8)")

    # 校准数据
    parser.add_argument("--calib-data", type=str, default=None,
                        help="校准数据文件路径 (.txt/.json/.jsonl)")
    parser.add_argument("--calib-samples", type=int, default=128,
                        help="校准样本数量 (default: 128)")
    parser.add_argument("--max-length", type=int, default=512,
                        help="校准数据最大长度 (default: 512)")
    ...
    if not is_compatible:
        log.error(f"模型兼容性检查失败: {message}")
        return 1

    # 获取模型信息
    model_info = get_model_info(args.model_path)
    log.info(f"模型信息: {model_info}")
    ...
    try:
        ...
        log.info(f"量化配置: {validated_config}")
    ...
    if args.method in ['awq', 'smoothquant']:
        ...
        if args.calib_data:
            ...
            try:
                ...
                log.info(f"加载校准数据: {len(calibration_data)} 个样本")
            except Exception as e:
                log.error(f"加载校准数据失败: {e}")
                log.info("将使用默认校准数据")
    ...
    # 执行量化
    log.info(f"开始使用 {args.method.upper()} 方法量化模型...")
    ...
    try:
        ...
        log.info(f"量化完成! 耗时: {quantization_time:.2f}s")
        log.info(f"量化模型保存至: {output_path}")
    ...
    if not args.no_verify:
        ...
        try:
            ...
            if detected_type == args.method:
                log.info(f"✅ 量化类型验证通过: {detected_type}")
            else:
                log.warning(f"⚠️ 量化类型不匹配: 期望 {args.method}, 检测到 {detected_type}")
            ...
            log.info(f"原始模型大小: {original_size / (1024 ** 3):.2f} GB")
            log.info(f"量化模型大小: {quantized_size / (1024 ** 3):.2f} GB")
            log.info(f"压缩比: {compression_ratio:.2f}x")

        except Exception as e:
            log.warning(f"量化验证失败: {e}")
