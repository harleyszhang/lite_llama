...
完整表格见 [`bench_quant_gemm_h100_20260901.json`](bench_quant_gemm_h100_20260901.json)。一个**测试点**就是一个「模型 × 投影 × token 数 M」的组合：两个模型（Qwen3-4B、Qwen3-30B-A3B）各取 4 个真实投影（qkv / o / gate_up / down），M 取 `{1, 8, 32, 128, 512, 2048}` 六档，2 × 4 × 6 = **48 个测试点**；每点测 7 行实现（6 种格式加 1 行消融），共 336 行。运行时设 `LITE_LLAMA_AUTOTUNE=0`。
...
Qwen3-30B-A3B 几何（E=128, top_k=8, h=2048, i=768），单位 µs，运行时设 `LITE_LLAMA_AUTOTUNE=0`——即用户在没有调优缓存时拿到的启发式 tile。括号内为相对 fp16 基线的加速比（fp16 耗时 ÷ 该格式耗时），大于 1 快于基线；「仅 `moe_align`」是消融行、不含 GEMM 计算，不参与对比；末列旧 tile 是 §1.3 的回归哨兵，其比值就是旧缺陷的量级。数据来自 [`bench_fused_moe_h100_20260901.json`](bench_fused_moe_h100_20260901.json)：

| tokens | fp16（基线） | fp8 W8A16 | fp8 W8A8 | int8 | int4 | 仅 `moe_align` | fp16 @ 旧 BLOCK_K=32 |
|---|---|---|---|---|---|---|---|
| 1 | 360.7 | 365.2 (0.99×) | 481.1 (0.75×) | 364.3 (0.99×) | 366.2 (0.98×) | 188.2 | 359.6 (1.00×) |
| 8 | 363.7 | 368.5 (0.99×) | 481.5 (0.76×) | 367.3 (0.99×) | 367.2 (0.99×) | 186.6 | 363.8 (1.00×) |
| 64 | 531.7 | 439.1 (1.21×) | 484.8 (1.10×) | **398.9 (1.33×)** | 630.0 (0.84×) | 180.9 | 713.4 (0.75×) |
| 512 | 583.4 | 615.5 (0.95×) | **477.8 (1.22×)** | 576.9 (1.01×) | 691.8 (0.84×) | 182.0 | 753.9 (0.77×) |
| 4096 | 1573.2 | 2301.0 (0.68×) | **1469.4 (1.07×)** | 2096.7 (0.75×) | 2598.7 (0.61×) | 217.2 | 1755.5 (0.90×) |

三种负载区间，没有一种格式能赢下超过一个：

- **≤8 tokens**：fp16 与三种 weight-only 格式全部落在 1.5% 之内，尽管读的权重字节差 4×——因为仅 `moe_align_block_size` 一项就约 188 µs，超过该层总耗时的一半，对每种格式完全相同，是这条路径上最大的单一目标。W8A8 是唯一能分辨出来的行，而且*差* 33%：在一个本就 launch 受限的层上多出两次量化 launch。
...
`_launch_config` 原来返回 `BLOCK_K = 128 if quant_mode else 32`。tile 搜索（`bench_fused_moe.py --tune`）在所有 token 档都**找不到**任何 `BLOCK_K` 低于 64 的获胜 fp16 配置；窄 tile 在 t64 损失 25.5%、t512 损失 22.6%、t4096 损失 10.4%。它压低的从来只是*未量化基线*——所以没有任何测试抓住它，t512 的 W8A16 fp8 也一度被读成 18% 的赢面而非上表的 5.5% 落败。已修复：所有模式都用 128。表格最后一列保留旧 tile 作为回归哨兵——两列 fp16 收敛即说明旧问题没有回来。
