"""Public API surface: lazy imports keep ``import rapid_llm`` CUDA-free.

Only the names in ``__all__`` — :class:`~rapid_llm.engine.llm.LLM`, the
engines, :class:`~rapid_llm.engine.sampler.SamplingParams` — are exposed,
