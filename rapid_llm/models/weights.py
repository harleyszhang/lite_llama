"""HuggingFace checkpoint keys -> rapid_llm parameters.
...
#: ``lm_head.weight`` is absent because :class:`~rapid_llm.modules.vocab_parallel.
...
def translate_text_key(key: str, packed: Mapping[str, tuple[str, ...]]) -> Target:
    ...
    Returns:
        ...
        fills. Keys that already match a rapid_llm parameter name
...
def load_weights(
    ...
    Args:
        model: Model whose parameters have real storage (see
            :func:`rapid_llm.executor.loader.materialise_parameters`).
        ...
        tied: ``{target parameter: source parameter}`` pairs to satisfy by
            *aliasing* when the checkpoint omits the target. Checkpoints with
            ...
            and rapid_llm keeps it as its own parameter, so it is pointed at
