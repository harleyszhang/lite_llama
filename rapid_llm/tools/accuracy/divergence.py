...
``transformers`` model and through rapid_llm's, hooks every decoder layer's
...
Usage:
    rapid-llm acc.divergence --model-dir my_weight/Qwen3-0.6B
    # or, as a library:
...
class DivergenceChecker:
    """Locates the first decoder layer where rapid_llm leaves its HF reference.
    ...
    :class:`~rapid_llm.tools.harness.SingleLayerHarness` exists for —
    :meth:`harness_for` hands the named layer straight to it.

    Args:
        config: Parsed model config.
        lite_model: The rapid_llm model, loaded and in eval mode.
...
def find_first_divergent_layer(
    ...
    *,
    ...
    The one-call entry point behind ``rapid-llm acc.divergence``: loads both
