...
class HFLayerReference(LayerReference):
    ...
    Args:
        config: The parsed rapid_llm config; its ``text_config`` drives the layer.
    ...
    def state_dict(self) -> dict[str, torch.Tensor]:
        ...
        persistent has no counterpart in the rapid_llm layer, and the strict coverage
    ...
    def _forward(
        ...
        if self._freqs_cis:
            ...
            half = cos.shape[-1] // 2
            position_embeddings = torch.complex(cos[..., :half].float(), sin[..., :half].float())
        with torch.no_grad():
            out = self.layer(
                hidden_states,
                attention_mask=mask,
