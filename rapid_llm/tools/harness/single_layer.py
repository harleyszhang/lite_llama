...
class SingleLayerCache:
    ...
    :class:`~rapid_llm.executor.model_runner.ModelRunner` derives all of this from a
    profiled :class:`~rapid_llm.executor.kv_cache_manager.KVCacheManager`. Here the
    ...
    def step_decode(self) -> AttentionMetadata:
        ...
        :meth:`~rapid_llm.executor.model_runner.ModelRunner.decode_alloc_kv_cache`: the
...
class ModuleTimer:
    ...
    module tree :mod:`rapid_llm.tools.profiling` prints, and there is no trace to
...
class LayerReference:
    ...
    import: :class:`~rapid_llm.tools.harness.reference.HFLayerReference` is the one
    ...
    def load_weights(self, checkpoint: Iterable[tuple[str, torch.Tensor]], source: str) -> None:
        ...
        Raises:
            ValueError: If any of the layer's parameters ends up unfilled or half
                filled. Inherited from :func:`rapid_llm.models.weights.load_weights`,
