...
and the guarantee in one run. ``--backend`` swaps the measured system
(lite / hf / vllm) so the same workload is comparable across engines.

Feature-level A/B (the MoE router GEMM variant, L1 cross-stream overlap)
lives in ``bench_optimizations.py``'s feature matrix, not here: this script
is the cross-engine baseline plus the eager/graph parity guarantee.

Usage:
    python benchmarks/bench_e2e.py --model-dir <ckpt> --verify
    python benchmarks/bench_e2e.py --model-dir <ckpt> --backend vllm
"""
