...
Usage:
    python benchmarks/kernels/bench_tbo_cost_model.py --batch 16
"""
...
def main() -> int:
    ...
    w_qkv = torch.randn(
        HIDDEN, (N_HEADS + 2 * 2) * HEAD_DIM // TP, device=dev, dtype=torch.bfloat16
    )
    ...
    # reasoned about (real AR needs the TP2 engine, measured by the L2 bench).
    ...
    print("NOTE: AR wire time must be measured on the TP2 engine (NCCL), not here.")
    return 0
