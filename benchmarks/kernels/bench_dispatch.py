...
def main() -> int:
    ...
    ap.add_argument("--iters", type=int, default=200, help="Rounds per cold/warm case; median")
    ap.add_argument("--json", default=None)
    args = ap.parse_args()

    print(f"dtype={DTYPE}, {args.iters} iters (first runs once and includes backend detection)\n")
    results = {
    ...
    print(
        f"\nwarm breakdown ({op}): platform snapshot {parts['platform_detect_us']:.2f} us + "
        f"env lookup {parts['env_lookup_us']:.2f} us + cache hit {parts['cache_hit_us']:.3f} us"
    )
    ...
    print(
        f"the most expensive filter+rank {worst_cold:.1f} us = "
        f"{worst_cold / step_us * 100:.2f}% of one decode step; the cache-hit one "
        f"{worst_warm:.2f} us = {worst_warm / step_us * 100:.3f}%."
        "\nThe call site decides once at construction and stores it as an attribute, so "
        "per-step forward does not even pay this lookup."
    )
