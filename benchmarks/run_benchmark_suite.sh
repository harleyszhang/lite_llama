...
# (rapid_llm vs HF transformers,TTFT/TPOT/TGS 口径),每档两个 batch 配置,
...
run Qwen3-8B "8:128" --engine rapid_llm --max-gpu-num-blocks 16384
run Meta-Llama-3.1-8B-Instruct "8:128" --engine rapid_llm --max-gpu-num-blocks 16384
...
# 需 ~60 GB,双卡 44 GB 放不下,只测 rapid_llm 单侧(decode eager)。
run_tp Qwen3-30B-A3B-Instruct-2507-FP8 "8:128 16:128" --engine rapid_llm
# AWQ:transformers 反量化需要 gptqmodel/autoawq(本机未装),只测 rapid_llm 单侧;
# b16 档单卡放得下(W4 权重 9.6 GiB + 1 GiB KV + graph 捕获工作区)。
run Qwen3-14B-AWQ "8:128 16:128 16:256" --engine rapid_llm
